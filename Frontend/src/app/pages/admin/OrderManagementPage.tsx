...
import {
  ...
  Eye, ChevronDown, ArrowUpDown, AlertTriangle, RefreshCw, ShoppingBag,
} from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="p-5 space-y-5 flex-1 overflow-y-auto">
          ...
          <div>
            ...
            <div className="bg-white border border-gray-100 rounded-xl p-4 space-y-1.5">
              ...
              {order.address && (
                <div className="text-sm text-gray-500 pt-1 border-t border-gray-100">{order.address}</div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-2">
                {order.items.map((item, i) => (
                  <div key={i} className="flex justify-between items-center p-3 bg-gray-50 rounded-xl">
                    ...
                    <div className="font-semibold text-gray-900 text-sm">{formatPrice(item.price * item.qty)}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {!STATUS_CONFIG_VALID ? (
              ...
              <>
                <div className="relative">
                  <select
                    value={localStatus}
                    onChange={(e) => { setLocalStatus(e.target.value as OrderStatus); setActionError(null); }}
                ...
                {localStatus !== order.status && (
                  <div className="mt-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 p-2.5 rounded-lg">
                    Changing: <strong>{statusConfig[order.status]?.label}</strong> → <strong>{statusConfig[localStatus]?.label}</strong>
                  </div>
                )}
              </>
            )}
        ...
        <div className="flex gap-3 p-5 border-t border-gray-100 flex-shrink-0">
          ...
          <button
            onClick={handleSave}
            disabled={saving || !STATUS_CONFIG_VALID}
...
export default function OrderManagementPage() {
  const { orders: rawOrders, loading, error, updateStatus, refetch } = useOrders();
  ...
  const handleStatusChange = async (id: string, status: OrderStatus) => {
    const numId = Number(id.replace("ALE-ORDER-", ""));
    if (!numId) throw new Error("Order action is not configured correctly.");
    await updateStatus(numId, status);
  };
    ...
    <div className="space-y-5">
      ...
      {rowActionError && (
        <div className="flex items-center justify-between gap-3 px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700">
          ...
          <button onClick={() => setRowActionError(null)} className="p-1 hover:bg-red-100 rounded-lg">
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
            statusFilter === "all" ? "bg-[#7C2D12] text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-[#7C2D12]"
          }`}
        >
          All ({orders.length})
        </button>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 border border-gray-200 bg-white rounded-xl text-sm text-gray-600 hover:border-[#7C2D12] transition-colors"
        >
      ...
      {selectedOrder && (
        <OrderDrawer
          order={selectedOrder}
          onClose={() => { setSelectedOrder(null); setRowActionError(null); }}
