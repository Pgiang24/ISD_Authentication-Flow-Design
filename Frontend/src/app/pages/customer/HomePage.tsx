      ...
      <div className="p-4">
        ...
        {product.isCombo && product.comboItems && product.comboItems.length > 0 && (
          <div className="mt-3 border-t border-gray-100 pt-3">
            <button onClick={() => setComboOpen((prev) => !prev)}
                  ...
                  <div key={item.id} className="flex items-start gap-3 p-2.5 bg-purple-50 rounded-xl border border-purple-100">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {item.price > 0 && <span className="text-xs font-bold text-[#d35f1a] flex-shrink-0">{formatPrice(item.price)}</span>}
                      </div>
                      <div className="text-xs text-gray-500 mt-0.5 line-clamp-1">{item.description}</div>
                      {item.weight && <div className="text-xs text-purple-600 font-medium mt-0.5">{item.weight}</div>}
                    </div>
                  </div>
                ))}
              </div>
...
export default function HomePage() {
  ...
  const stats = [
    { icon: Flame,       label: t("stats.smoke"),    value: t("stats.smokeVal"),    sub: t("stats.smokeSub"),    color: "text-orange-500 bg-orange-50" },
    { icon: MapPin,      label: t("stats.origin"),   value: t("stats.originVal"),   sub: t("stats.originSub"),   color: "text-[#2D6A4F] bg-emerald-50" },
    { icon: ShieldCheck, label: t("stats.cert"),     value: t("stats.certVal"),     sub: t("stats.certSub"),     color: "text-blue-600 bg-blue-50" },
    { icon: Clock,       label: t("stats.delivery"), value: t("stats.deliveryVal"), sub: t("stats.deliverySub"), color: "text-[#D4A853] bg-amber-50" },
  ];
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden min-h-[520px] flex items-center justify-center">
        ...
        <div className="relative z-10 mx-4 my-12 px-8 py-8 rounded-2xl text-center max-w-md" style={{ background: "rgba(211, 95, 26, 0.80)" }}>
          <p className="text-white/80 text-xs uppercase tracking-widest font-semibold mb-2">{t("hero.tag")}</p>
          <h1 className="text-3xl md:text-4xl font-black leading-tight mb-3 uppercase tracking-tight">
            <span className="text-[#ffe0c8]">{t("hero.title1")}</span><br />
            <span className="text-white">{t("hero.title2")}</span>
          </h1>
          <p className="text-white/80 text-sm leading-relaxed mb-6">{t("hero.desc")}</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <a href="#featured" className="px-8 py-3 border-2 border-white/70 text-white rounded-full font-bold text-sm uppercase tracking-widest transition-all hover:bg-white/15 active:scale-95">
              {t("hero.viewFeatured")}
            </a>
          </div>
        </div>
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-[#1C0A00]/88 via-[#1C0A00]/55 to-transparent flex items-center">
          <div className="max-w-6xl mx-auto px-6 md:px-10 w-full">
            <div className="max-w-xs md:max-w-sm">
              <p className="text-[#D4A853] text-[10px] font-bold uppercase tracking-widest mb-2">{t("about.founded")}</p>
              <h2 className="text-white font-black text-2xl md:text-3xl mb-2 leading-tight">{t("about.title")}</h2>
              <p className="text-white/70 text-sm leading-relaxed mb-4">{t("about.desc")}</p>
      ...
      <section id="featured" className="max-w-6xl mx-auto px-6 py-14">
        <div className="flex items-end justify-between mb-8">
          <div>
            <p className="text-[#d35f1a] italic text-sm font-medium mb-1">{t("featured.tag")}</p>
            <h2 className="text-3xl font-black text-gray-900 tracking-tight">{t("featured.title")}</h2>
          </div>
        ...
        <div className="text-center mt-10">
          <Link to="/products" className="inline-flex items-center gap-2 px-10 py-4 bg-[#7C2D12] hover:bg-[#6B2510] text-white rounded-full font-bold text-sm transition-all hover:scale-105 shadow-lg shadow-[#7C2D12]/20">
            {t("featured.viewAllBtn")} <ChevronRight className="w-4 h-4" />
          </Link>
        </div>
      </section>
    </div>
